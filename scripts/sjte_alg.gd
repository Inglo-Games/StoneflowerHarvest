...
static func sjte(N):
	...
	while has_mobile:
		...
		max_mobile += directions[max_mobile]
		...
		elif indicies[max_mobile] < indicies[max_mobile+directions[max_mobile]]:
			directions[max_mobile] = 0
		...
		# DEBUG: Print current permutation
		print("Current permutation: " + str(indicies))
